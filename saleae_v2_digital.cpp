...
static void enqueue_transition(logic_t *logic, int ch, double ts) {
  ...
  for (curr = logic->transitions; ts > curr->timestamp;
...
static void refill_queue(logic_t *logic, const int *channels,
...
static int channel_init(logic_t *logic, int ch) {
  mf_t *mf = logic->mf + ch;
  const saleae_v2_digital_t *hdr =
      reinterpret_cast<const saleae_v2_digital_t *>(mf->mem);
  const uint8_t *ptr =
      reinterpret_cast<const uint8_t *>(mf->mem) + sizeof(saleae_v2_digital_t);
  int res = 0;
  ...
  if (((mf->len - sizeof(saleae_v2_digital_t)) / sizeof(double)) !=
      hdr->num_transitions) {
  ...
  fprintf(stderr,
          "found channel %2d - initial state: %x, %12" PRIu64 " transitions\n",
...
int logic_init(logic_t *logic, const char *file_prefix) {
  ...
  char *fn_buf = reinterpret_cast<char *>(alloca(fn_len + 8));
  int i, res;
  ...
  /* add extra transition buffer to pool - needed for pivoting during
   * remove-first */
  logic->trans_stack[logic->trans_stack_idx++] = logic->trans_pool + i;
 ...
 */
int logic_replay(logic_t *l, uint32_t *state, double *ts, uint32_t trig_falling,
                 uint32_t trig_rising) {
  uint32_t last_state, new_state;
  int res = 0;

  for (last_state = l->state; (res = logic_step(l, &new_state, ts));
       last_state = new_state) {
    uint32_t changed = last_state ^ new_state;
    if ((changed & new_state & trig_rising) ||
        (changed & last_state & trig_falling))
      break;
  }
